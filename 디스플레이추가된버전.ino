...
unsigned long lastScreenSwitch = 0; // 현재는 사용 안 하지만 남겨둠
int screenMode = 1;
int lastAnimSec = -1;
...
void handle_S00_WaitSignal() {
  ...
  if (Serial.available() > 0) {
    ...
    upper.toUpperCase();
    if (upper == "START") {
      Serial.println(">> 차량 인식, START 수신.");
      changeState(S01_WAIT_INPUT);
    }
  }
}

// S01: 목표 충전량 입력 대기 (로직은 그대로, UI만 변경)
//  - lcd1: "VEHICLE DETECTED"
//  - lcd2: "WAIT UNTIL APPROVE"
void handle_S01_WaitInput() {
  if (isStateFirstRun) {
    lcd1.clear();
    lcd2.clear();
    lcd1.setCursor(0, 0); lcd1.print("VEHICLE DETECTED");
    lcd2.setCursor(0, 0); lcd2.print("WAIT UNTIL APPROVE");
...
// S1: 승인 대기 (10초)
//  - lcd1: APPROVING, APPROVING., APPROVING.., APPROVING...
//  - lcd2: "Please Wait Xs"
void handle_S1_Approving() {
  if (isStateFirstRun) {
...
void handle_S2_Approved() {
  if (isStateFirstRun) {
    ...
    lcd1.clear();
    lcd2.clear();
    lcd1.setCursor(0, 0); lcd1.print("APPROVED!");
    lcd2.setCursor(0, 0); lcd2.print("Ready to Align");
    Serial.println("\n[S2] 승인 완료.");
    isStateFirstRun = false;
  }
...
//  - 시작: lcd1 = "HELLO XXXX", lcd2 = "ALIGNING..."
//  - 완료: lcd1 = "HELLO XXXX", lcd2 = "ALIGN COMPLETE"
void handle_S3_Aligning() {
  if (isStateFirstRun) {
    Serial.println("\n[S3] 정렬 시작...");
    isStateFirstRun = false;
...
// S4: 충전
//  - lcd1: CHARGING, CHARGING., CHARGING.., CHARGING...
//  - lcd2: "Q: xxxx mAh" (누적 충전량)
//  - 종료 조건: 기존 로직 그대로 (Q_target_mAh 또는 10초 디버그)
void handle_S4_Charging() {
  if (isStateFirstRun) {
  ...
  // 1초마다 LCD 업데이트 & 로그
  int elapsedSec = (currentMillis - stateStartTime) / 1000;
  if (elapsedSec != lastAnimSec) {
    lastAnimSec = elapsedSec;
...
void handle_S5_Completed() {
  ...
  if (millis() - stateStartTime > 2000) {
    // d1,d4 기준으로 정지 위치 맞추기 (로직 그대로 유지)
