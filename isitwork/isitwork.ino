...
void handle_S00_WaitSignal() {
  ...
  if (Serial.available() > 0) {
    ...
    if (trySetPlateFromLine(line)) {
      // 번호판만 받아도 다음 단계로 진행 (TARGET 입력 대기)
...
void handle_S01_WaitInput() {
  ...
  if (Serial.available() > 0) {
    ...
    Serial.print(">> 설정된 목표치: "); Serial.print((int)Q_target_mAh); Serial.println(" mAh");
